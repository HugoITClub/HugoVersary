async function loadActivities() {
  ...
  const response = await getHCActivities(0, 1000);
...
async function loadHCTopPic() {
  const response = await getHCInformation(0, 1000);
  return new Promise((resolve) => {
          ...
          return (
            <React.Fragment>
              <div
                ...
                id="header-pic"
                style={{
                  backgroundImage: `linear-gradient(180deg, rgba(0, 0, 0, 0) 60%, #121212 83%), url(${topPicUrl})`,
                  objectFit: "cover",
                }}
              >
                <div className="team-text-content  text-align-center">
                  <p className="text-uppercase team-name">heroes company</p>
                  <p className="team-name-slogan">“Stronger Together.”</p>
                </div>
              </div>
            </React.Fragment>
          );
...
async function loadHCTeamActivities() {
  const response = await getHCInformation(0, 1000);
  return new Promise((resolve) => {
              ...
              <div className="container">
                <div className="team-information-text row text-center">
                  <h1 className="mt-5 team-information-text-title">
                    Explore <span className="hc-color">{teamName}</span>
                  </h1>
...
async function loadHCBottomPic() {
  const response = await getHCInformation(0, 1000);
  return new Promise((resolve) => {
...
async function run() {
  ...
  await loadHCTopPic();
  await loadHCBottomPic();
  await loadHCTeamActivities();
  setupOwlCarousel2();
}
