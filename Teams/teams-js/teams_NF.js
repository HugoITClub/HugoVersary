async function loadActivities() {
  ...
  const response = await getNFActivities(0, 1000);
...
async function loadNFTopPic() {
  const response = await getNFInformation(0, 1000);
  return new Promise((resolve) => {
            ...
            <React.Fragment>
              ...
              >
                <div className="team-text-content  text-align-center">
                  <p className="text-uppercase team-name">Niffler</p>
                  <p className="team-name-slogan">“Friends Forever.”</p>
                </div>
              </div>
            </React.Fragment>
          );
...
async function loadNFTeamActivities() {
  const response = await getNFInformation(0, 1000);
  return new Promise((resolve) => {
              ...
              <div className="container">
                <div className="team-information-text row text-center">
                  <h1 className="mt-5 team-information-text-title">
                    Explore <span className="nf-color">{teamName}</span>
                  </h1>
...
async function loadNFBottomPic() {
  const response = await getNFInformation(0, 1000);
  return new Promise((resolve) => {
...
async function run() {
  ...
  await loadNFTopPic();
  await loadNFBottomPic();
  await loadNFTeamActivities();
  setupOwlCarousel2();
}
